...
個人事業主（岡﨑 好信 / 岡﨑ＩＴコンサルティング株式会社）の「名刺代わり」1ページ静的サイト。HTML / CSS / JS のみ・モバイルファースト。**保守フェーズ（残タスクなし）**。
...
- 公開対象: `index.html` / `404.html` / `css/` / `js/` / `images/` / favicon類 / `ogp.jpg` / `robots.txt` / `sitemap.xml`
- 非公開: `client_source/`（依頼主の私的資料・`.gitignore` 済）

## 開発フロー（通常開発。旧「独立ハーネス」は 2026-07-14 廃止）
...
- 🔒 **個人情報ガード**：`client_source/`（顔写真の原本・職務経歴書・名刺PDF＝自宅住所/携帯番号）は `.gitignore` 済。加えて `.git/hooks/pre-commit` が `client_source/` 配下・秘密ファイル・携帯番号/郵便番号の混入をブロックする（**このPCローカルのみ＝別PCでは要再設置**）。**リポジトリは Public なので一度 push すると履歴から消せない**
- **残タスク＝なし**。独自ドメイン・WebP 化は「対応しない」で確定。名刺入稿データ（両面 55×91mm・PDF）は `client_source/meishi/card-source.html` を直して再出力すれば微調整可
- 完了済み Sprint（0〜8・A〜D）とデザイン全面刷新（2026-07-14・クライアント承認済み）の内訳は **git 履歴が正本**（ここには残さない）
